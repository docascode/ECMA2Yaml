...
namespace ECMA2Yaml
{
    public partial class ECMALoader
    {
        ...
        {
            var returnTypes = returnValueElement?.Elements("ReturnType").Select(r => new { ReturnType = r.Value, Monikers = r.Attribute("FrameworkAlternate")?.Value });
            if (returnTypes == null || !returnTypes.Any()) return null;

            Func<string, HashSet<string>> convertMonikers = fxlist =>
            {
                if (string.IsNullOrWhiteSpace(fxlist))
                    return new HashSet<string>();

                return new HashSet<string>(fxlist.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries).Distinct());
            };
            ...
            {
                Value = r.ReturnType,
                Monikers = convertMonikers(r.Monikers)
            }).Select(toParam);
